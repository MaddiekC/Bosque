...
class DetalleCorteController extends Controller
{
        ...
        try {
            $result = DB::transaction(function () use (
                ...
                foreach ($cabeceraIds as $cabId) {
                    ...
                    CabeceraCorte::where('id', $cabId)->update(['cant_trozas' => $cnt]);
                    $counts[$cabId] = $cnt;
                }
    ...
    public function uploadExcel(Request $request)
    {
        ...
        try {
            ...
            // actualizar campo cant_trozas en la cabecera usando COUNT (número de filas/records)
            $cabecera->cant_trozas = $newCount;
            $cabecera->save();
    ...
    public function destroyByCabecera($cabecera_corte_id)
    {
        $user = auth()->user();
        $updated = DetalleCorte::where('cabecera_corte_id', $cabecera_corte_id)
            ->where('estado', 'A')
            ->update([
                'estado' => 'I',
        ...
        CabeceraCorte::where('id', $cabecera_corte_id)->update(['cant_trozas' => 0]);

        return response()->json([
            'message' => 'Todos los detalles fueron marcados como inactivos',
            'count' => $updated
        ]);
    }
    ...
    public function reporteAcumulado($dateYear)
    {
        ...
        $rows = DetalleCorte::from('detalle_corte as d')
            ...
            ->whereIn('c.estado', ['A', 'C']) // solo cabeceras activas o cerradas
