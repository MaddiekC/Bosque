...
class CabeceraCorteController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            $cabeceraCorte = CabeceraCorte::create([
                ...
                'cant_trozas'   => $request->cant_trozas,
    ...
    public function update(Request $request, $id)
    {
        ...
        $cabeceraCorte->loadCount('detalleCortes');
        $cabeceraCorte->load(['contrato.cliente', 'raleoTipo']);

        // Adjuntar pares únicos bosque_id y siembra_rebrote_id desde detalle_corte
        $pares = DB::table('detalle_corte as d')
            ->select('d.cabecera_corte_id', 'd.bosque_id', 'd.siembra_rebrote_id')
            ->where('d.cabecera_corte_id', $cabeceraCorte->id)
            ->whereNotNull('d.bosque_id')
            ->whereNotNull('d.siembra_rebrote_id')
            ->groupBy('d.cabecera_corte_id', 'd.bosque_id', 'd.siembra_rebrote_id')
            ->get();

        if ($pares->isNotEmpty() && ($cabeceraCorte->bosque_id === null || $cabeceraCorte->siembra_rebrote_id === null)) {
            $cabeceraCorte->bosque_id = $pares->pluck('bosque_id')->unique()->values()->all();
            $cabeceraCorte->siembra_rebrote_id = $pares->pluck('siembra_rebrote_id')->unique()->values()->all();
        }
    ...
    public function destroy($id)
    {
        ...
        // Solo permitir eliminar si no tiene detalles o si todos sus detalles están inactivos (estado 'I')
        $tieneDetallesActivos = $cabeceraCorte->detalleCortes()
            ->where('estado', '!=', 'I')
            ->exists();

        if ($tieneDetallesActivos) {
            return response()->json([
                'message' => 'No se puede eliminar la cabecera porque tiene detalles de corte activos'
            ], 400);
        }
