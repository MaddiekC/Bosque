...
export class BosqueComponent implements AfterViewInit {
  ...
  constructor(
    private bosqueService: ApiService,
  ...
  async exportToPDF() {
    const columns = [
      { header: 'Nombre', dataKey: 'nombre', align: 'left' as const },
      { header: 'Sección', dataKey: 'seccion_id', align: 'center' as const },
      { header: 'Hectáreas', dataKey: 'hectarea', align: 'right' as const }
    ];

    const rows = this.bosquesFiltrados.map(item => ({
      nombre: item.nombre,
      seccion_id: this.getSeccionNombre(item.seccion_id),
      hectarea: this.pdfService.fmtNumber(item.hectarea, 2, 2)
    }));

    const filasTotales = [
      ['Total hectáreas:', '', this.pdfService.fmtNumber(this.totalHectareas, 2, 2)]
    ];

    await this.pdfService.exportarTabla({
      titulo: 'Reporte de Bosques',
      nombreArchivo: 'reporte_bosques.pdf',
      columnas: columns,
      datos: rows,
      filasTotales
    });
